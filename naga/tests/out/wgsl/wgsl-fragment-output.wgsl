struct FragmentOutputVec4Vec3_ {
    @location(0) @align(32) vec4f: vec4<f32>,
    @location(1) @interpolate(flat) @align(16) vec4i: vec4<i32>,
    @location(2) @interpolate(flat) @align(32) vec4u: vec4<u32>,
    @location(3) @align(16) vec3f: vec3<f32>,
    @location(4) @interpolate(flat) @align(64) vec3i: vec3<i32>,
    @location(5) @interpolate(flat) @align(16) vec3u: vec3<u32>,
}

struct FragmentOutputVec2Scalar {
    @location(0) vec2f: vec2<f32>,
    @location(1) @interpolate(flat) vec2i: vec2<i32>,
    @location(2) @interpolate(flat) @align(16) vec2u: vec2<u32>,
    @location(3) scalarf: f32,
    @location(4) @interpolate(flat) scalari: i32,
    @location(5) @interpolate(flat) @align(32) scalaru: u32,
}
